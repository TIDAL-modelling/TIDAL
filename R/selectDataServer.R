    ...
    function(input, output, session) {
      ...
      output$additional <- renderUI({
        ...
        tagList(
          ...
          selectInput(ns("traj"), "Variable to model trajectory on, eg. depression scores/height/income (continuous):", choices = names(data()) , selected = names(data())[4]),
          selectInput(ns("age"), "Variable for age/time at time point (continuous):", choices = names(data()) , selected = names(data())[3]),
                   ...
                   checkboxInput(ns("toYears"),
                                 tags$span("Convert age from months to years",
                                           tipify(bsButton("pB2", "?", style = "info", size = "extra-small"),
                                                  "")),
                                 value = FALSE )),
          selectInput(ns("timePoint"), "Variable for time point/occasion (categorical):", choices = names(data()) , selected = names(data())[2]),
          selectInput(ns("covarsCat"), "Categorical Confounders/Covariates (optional):",
                      choices = names(data()) ,
                      selected = NULL,
                      multiple = TRUE),
          selectInput(ns("covarsCont"), "Continuous Confounders/Covariates (optional):",
                      choices = names(data()) ,
                      selected = NULL,
                      multiple = TRUE),
