  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      interpretation <- eventReactive(fit(), {
        ...
        if (input$varType == "cat") {
          if(modelType() ==  "Linear")
            modelSpecificMessage <-  paste0('It is possible to estimate different trajectory groups by adding the intercept and ', ageVar, ' estimates to the corresponding group effect estimate and ', ageVar, ':group-interaction estimates to get group specific trajectories. If you are interested in the effect of the interaction only, then you can ignore the intercept and ', ageVar, ' estimates and focus solely on the corresponding interaction and ', ageVar, ':group-interaction estimates.')
          else if(modelType() ==  "Quadratic"){
            modelSpecificMessage <- paste0('As you have specified a quadratic model, every unit increase in ', ageVar ,'^2 is also associated with ', direction2 ,' of ', trajVar,' by ',abs(slope2),'. It is possible to estimate different trajectory groups by adding the intercept, ', ageVar, ' and ', ageVar ,'^2 estimates to the corresponding group effect estimate, ', ageVar, ':group-interaction and ',ageVar, '^2:group-interaction estimates to get group specific trajectories. If you are interested in the effect of the interaction only, then you can ignore the intercept, ', ageVar, ' and ', ageVar ,'^2 estimates and focus solely on the corresponding  interaction, ', ageVar, ':group-interaction and ',ageVar, '^2:group-interaction estimates.<br/> However, it can be difficult to interpret these estimates in isolation, so we would recommend exploring your trajectories with the "Plot" and "Scores At Ages" tabs for more information.')
          }else if(modelType() ==  "Cubic"){
            modelSpecificMessage <- paste0('As you have specified a cubic model, every unit increase in ',ageVar,'^2 is also associated with ', direction2 ,' of ', trajVar ,' by ', abs(slope2) ,'. Furthermore, every unit increase in ',ageVar,'^3 is also associated with ', direction3 ,' of ', trajVar,' by ',abs(slope3),'. It is possible to estimate different trajectory groups by adding the intercept, ', ageVar, ', ', ageVar ,'^2 and ', ageVar ,'^3 estimates to the corresponding group effect estimate, ', ageVar, ':group-interaction, ',ageVar, '^2:group-interaction and ',ageVar, '^3:group-interaction  estimates  to get group specific trajectories. If you are interested in the effect of the interaction only, then you can ignore the intercept, ', ageVar, ', ', ageVar ,'^2 and ', ageVar ,'^3 estimates and focus solely on the corresponding  interaction, ', ageVar, ':group-interaction, ',ageVar, '^2:group-interaction and ',ageVar, '^3:group-interaction estimates.<br/>However, it can be difficult to interpret these estimates in isolation, so we would recommend exploring your trajectories with the "Plot" and "Scores At Ages" tabs for more information.')
          }else if (modelType() ==  "Quartic"){
            modelSpecificMessage <-  paste0('As you have specified a quartic model, every unit increase in ',ageVar,'^2 is also associated with ', direction2 ,' of ', trajVar ,' by ', abs(slope2) ,'. Furthermore, every unit increase in ',ageVar,'^3 is also associated with ', direction3 ,' of ', trajVar,' by ',abs(slope3),' and every unit increase in ',ageVar,'^4 is associated with ', direction4 ,' in ',trajVar,' by ',abs(slope4), '. It is possible to estimate different trajectory groups by adding the intercept, ', ageVar, ', ', ageVar ,'^2, ', ageVar ,'^3 and ', ageVar ,'^4 estimates to the corresponding group effect estimate, ', ageVar, ':group-interaction, ',ageVar, '^2:group-interaction, ',ageVar, '^3:group-interaction and ',ageVar, '^4:group-interaction  estimates to get group specific trajectories. If you are interested in the effect of the interaction only, then you can ignore the intercept, ', ageVar, ', ', ageVar ,'^2, ', ageVar ,'^3 and ', ageVar ,'^4 estimates and focus solely on the corresponding  interaction, ', ageVar, ':group-interaction, ',ageVar, '^2:group-interaction, ',ageVar, '^3:group-interaction and ',ageVar, '^4:group-interaction estimates.<br/> However, it can be difficult to interpret these estimates in isolation, so we would recommend exploring your trajectories with the "Plot" and "Scores At Ages" tabs for more information.')
          }
        } else if (input$varType == "cont") {

          if(modelType() ==  "Linear")
            modelSpecificMessage <-  paste0('<br><br>It is possible to estimate the trajectory of the outcome (', trajVar, ') across ', ageVar, ' for different values of the continuous interaction variable (', input$condition, '). You can do this by adding the intercept and ', ageVar, ' estimates to the corresponding main effect estimates of the continuous interaction variable, and then the estimates for the ', ageVar, ':', input$condition, ' interactions, which can be calculated for specific values of the continuous interaction variable. These calculated estimates will provide the trajectories for different levels of the continuous interaction variable.<br><br>If you are interested in the effect of the interaction only, you can focus solely on the corresponding interaction and ', ageVar, ':', input$condition, ' interaction estimates, as this captures how the relationship between ', ageVar, ' and the outcome variable changes with changes in the continuous interaction variable.')
          else if(modelType() ==  "Quadratic"){
            modelSpecificMessage <- paste0('As you have specified a quadratic model, every unit increase in ', ageVar ,'^2 is also associated with ', direction2 ,' of ', trajVar,' by ',abs(slope2),'.<br><br>It is possible to estimate the trajectory of the outcome (', trajVar, ') across ', ageVar, ' for different values of the continuous interaction variable (', input$condition, '). You can do this by adding the intercept, ', ageVar, ' and ', ageVar ,'^2 estimates to the corresponding main effect estimates of the continuous interaction variable, and then the estimates for the ', ageVar, ':', input$condition, ' and ',ageVar, '^2:', input$condition, ' interactions, which can be calculated for specific values of the continuous interaction variable. These calculated estimates will provide the trajectories for different levels of the continuous interaction variable.<br><br>If you are interested in the effect of the interaction only, you can focus solely on the corresponding interaction, ', ageVar, ':', input$condition, ' and ',ageVar, '^2:', input$condition, ' interaction estimates, as this captures how the relationship between ', ageVar, ' and the outcome variable changes with changes in the continuous interaction variable.')
          }else if(modelType() ==  "Cubic"){
            modelSpecificMessage <- paste0('As you have specified a cubic model, every unit increase in ',ageVar,'^2 is also associated with ', direction2 ,' of ', trajVar ,' by ', abs(slope2) ,'. Furthermore, every unit increase in ',ageVar,'^3 is also associated with ', direction3 ,' of ', trajVar,' by ',abs(slope3),'.<br><br>It is possible to estimate the trajectory of the outcome (', trajVar, ') across ', ageVar, ' for different values of the continuous interaction variable (', input$condition, '). You can do this by adding the intercept, ', ageVar, ', ', ageVar ,'^2 and ', ageVar ,'^3 estimates to the corresponding main effect estimates of the continuous interaction variable, and then the estimates for the ', ageVar, ':', input$condition, ', ',ageVar, '^2:', input$condition, ' and ',ageVar, '^3:', input$condition, ' interactions, which can be calculated for specific values of the continuous interaction variable. These calculated estimates will provide the trajectories for different levels of the continuous interaction variable.<br><br>If you are interested in the effect of the interaction only, you can focus solely on the corresponding interaction, ', ageVar, ':', input$condition, ', ',ageVar, '^2:', input$condition, ' and ',ageVar, '^3:', input$condition, ' interaction estimates, as this captures how the relationship between ', ageVar, ' and the outcome variable changes with changes in the continuous interaction variable.')
          }else if (modelType() ==  "Quartic"){
            modelSpecificMessage <-  paste0('As you have specified a quartic model, every unit increase in ',ageVar,'^2 is also associated with ', direction2 ,' of ', trajVar ,' by ', abs(slope2) ,'. Furthermore, every unit increase in ',ageVar,'^3 is also associated with ', direction3 ,' of ', trajVar,' by ',abs(slope3),' and every unit increase in ',ageVar,'^4 is associated with ', direction4 ,' in ',trajVar,' by ',abs(slope4), '.<br><br>It is possible to estimate the trajectory of the outcome (', trajVar, ') across ', ageVar, ' for different values of the continuous interaction variable (', input$condition, '). You can do this by adding the intercept, ', ageVar, ', ', ageVar ,'^2, ', ageVar ,'^3 and ', ageVar ,'^4 estimates to the corresponding main effect estimates of the continuous interaction variable, and then the estimates for the ', ageVar, ':', input$condition, ', ',ageVar, '^2:', input$condition, ', ',ageVar, '^3:', input$condition, ' and ',ageVar, '^4:', input$condition, ' interactions, which can be calculated for specific values of the continuous interaction variable. These calculated estimates will provide the trajectories for different levels of the continuous interaction variable.<br><br>If you are interested in the effect of the interaction only, you can focus solely on the corresponding interaction, ', ageVar, ':', input$condition, ', ',ageVar, '^2:', input$condition, ', ',ageVar, '^3:', input$condition, ' and ',ageVar, '^4:', input$condition, ' interaction estimates, as this captures how the relationship between ', ageVar, ' and the outcome variable changes with changes in the continuous interaction variable.')
            }

        }

        covarsMessage <- paste0(ifelse(length(covars()) > 0, paste0('In addition, the following confounders/covariates: ', confounderLevels, ' are associated with an increase or decrease of ', trajVar, ' by ', confounderEst, ' respectively. (Please note that continuous covariates are averaged across the values of that covariate within the sample. Categorical covariates are set to their lowest level by default.) <br/><br/>'), ''), '')
